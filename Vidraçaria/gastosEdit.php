...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "UPDATE gastos SET categoria = '$categoria', descricao = '$descricao', valor = $valor WHERE id = $id";

    if ($mysqli->query($sql) === true) {
        header("Location: financeiro.php"); // Redireciona para a página principal
        exit();
    } else {
        echo "Erro ao atualizar gasto: " . $mysqli->error;
    }
}

if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $result = $mysqli->query("SELECT * FROM gastos WHERE id = $id");
    ...
    echo "ID do gasto não fornecido.";
    exit();
}
...
<div class="container">
    ...
    <form id="editar-form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        ...
        <select id="categoria" name="categoria">
            ...
            <option value="Combustível" <?= $gasto['categoria'] === 'Combustível' ? 'selected' : '' ?>>Combustível</option>
            <option value="Alimentação" <?= $gasto['categoria'] === 'Alimentação' ? 'selected' : '' ?>>Alimentação</option>
            <!-- Adicione mais categorias conforme necessário -->
        </select>
        <label for="descricao">Descrição:</label>
        <input type="text" id="descricao" name="descricao" required value="<?= $gasto['descricao'] ?>">
        <label for="valor">Valor:</label>
        <input type="number" id="valor" name="valor" step="0.01" required value="<?= $gasto['valor'] ?>">
